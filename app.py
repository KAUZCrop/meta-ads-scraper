...
# ─────────────────────────────────────────────
# API Key — Streamlit Secrets에서 안전하게 로드
# 로컬 테스트 시: .streamlit/secrets.toml에 ANTHROPIC_API_KEY = "sk-ant-..."
# Streamlit Cloud: 앱 Settings > Secrets에 동일하게 등록
# ─────────────────────────────────────────────
def get_api_key() -> str:
    try:
...
/* ── 스탯 카드 ── */
.stat-card{background:var(--bg);border:1.5px solid var(--border);border-radius:12px;padding:14px 16px;position:relative;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.04);transition:box-shadow .2s,border-color .2s;}
.stat-card:hover{border-color:var(--accent);box-shadow:0 4px 16px rgba(26,109,255,.10);}
...
.kw-tag{font-family:'DM Mono',monospace;font-size:11px;background:var(--tag);border:1px solid rgba(26,109,255,.2);color:var(--accent);padding:4px 10px;border-radius:20px;display:inline-flex;align-items:center;gap:6px;}
...
/* ── 소재 카드 ── */
.asset-card-wrap{border:1.5px solid var(--border);border-radius:12px;overflow:hidden;margin-bottom:12px;background:var(--bg);box-shadow:0 1px 4px rgba(0,0,0,.05);transition:box-shadow .2s,border-color .2s;}
.asset-card-wrap:hover{border-color:var(--accent);box-shadow:0 4px 20px rgba(26,109,255,.10);}
...
.badge-image {background:rgba(26,109,255,.1); color:var(--accent);  border:1px solid rgba(26,109,255,.2);}
.badge-video {background:rgba(91,61,255,.1);  color:#5b3dff;        border:1px solid rgba(91,61,255,.2);}
.badge-star  {background:rgba(224,123,0,.1);  color:var(--warn);    border:1px solid rgba(224,123,0,.2);}
.badge-ai    {background:rgba(0,168,107,.1);  color:var(--success); border:1px solid rgba(0,168,107,.2);}
.asset-meta  {font-size:11px;color:var(--muted);line-height:1.6;}
.asset-caption{font-size:11px;color:var(--text2);margin:4px 0 6px;line-height:1.4;opacity:.8;font-style:italic;}

/* ── AI 분석 박스 ── */
.ai-wrap{padding:0 12px 12px;background:var(--bg2);}
.ai-box{background:linear-gradient(135deg,rgba(26,109,255,.04),rgba(91,157,255,.06));border:1px solid rgba(26,109,255,.18);border-radius:10px;padding:10px 12px;}
.ai-box-header{font-family:'DM Mono',monospace;font-size:9px;color:var(--accent);letter-spacing:1px;text-transform:uppercase;margin-bottom:6px;}
.ai-box-body{font-size:12px;color:var(--text2);line-height:1.65;}
.ai-tag{display:inline-block;font-size:10px;font-weight:500;background:rgba(26,109,255,.1);color:var(--accent);border-radius:4px;padding:1px 6px;margin:2px 2px 0 0;}

/* ── AI 상태 배너 ── */
.ai-banner{display:flex;align-items:center;gap:10px;background:linear-gradient(90deg,rgba(26,109,255,.06),rgba(26,109,255,.02));border:1px solid rgba(26,109,255,.2);border-radius:10px;padding:10px 16px;margin-bottom:1rem;}
.ai-banner-warn{background:rgba(224,123,0,.06);border:1px solid rgba(224,123,0,.2);}
...
/* ── 버튼 ── */
.stButton>button{font-family:'DM Mono',monospace!important;font-size:11px!important;letter-spacing:.5px!important;border-radius:8px!important;border:1.5px solid var(--border2)!important;background:var(--bg)!important;color:var(--text2)!important;transition:all .15s!important;height:34px!important;box-shadow:0 1px 2px rgba(0,0,0,.05)!important;}
.stButton>button:hover{border-color:var(--accent)!important;color:var(--accent)!important;background:var(--accent-soft)!important;box-shadow:0 2px 8px rgba(26,109,255,.12)!important;}
...
/* ── 입력 ── */
.stTextInput input{background:var(--bg)!important;border:1.5px solid var(--border2)!important;border-radius:10px!important;color:var(--text)!important;font-family:'DM Sans',sans-serif!important;box-shadow:0 1px 3px rgba(0,0,0,.04)!important;}
.stTextInput input:focus{border-color:var(--accent)!important;box-shadow:0 0 0 3px rgba(26,109,255,.12)!important;}
div[data-baseweb="select"]>div{background:var(--bg)!important;border-color:var(--border2)!important;border-radius:8px!important;}

/* ── 알림 ── */
.stSuccess{background:rgba(0,168,107,.06)!important;border:1px solid rgba(0,168,107,.2)!important;border-radius:10px!important;}
.stWarning{background:rgba(224,123,0,.06)!important;border:1px solid rgba(224,123,0,.2)!important;border-radius:10px!important;}
.stInfo   {background:rgba(26,109,255,.06)!important;border:1px solid rgba(26,109,255,.2)!important;border-radius:10px!important;}
.stError  {background:rgba(232,40,74,.06)!important;border:1px solid rgba(232,40,74,.2)!important;border-radius:10px!important;}

/* ── 빈 상태 ── */
.empty-state{text-align:center;padding:80px 20px;color:var(--muted);}
.empty-icon {font-size:40px;margin-bottom:16px;opacity:.25;}
.empty-title{font-family:'Syne',sans-serif;font-size:16px;font-weight:600;margin-bottom:8px;color:var(--text);opacity:.4;}
...
# ─────────────────────────────────────────────
# AI 분석 — Secrets의 API Key 사용 (노출 없음)
# ─────────────────────────────────────────────
def analyze_ad(image_url: str, keyword: str) -> dict | None:
    if not API_KEY:
        return None
    try:
        prompt = f"""당신은 광고 전략 전문가입니다. Meta 광고 소재를 분석해주세요.

검색 키워드: {keyword}
이미지 URL: {image_url}

아래 JSON 형식으로만 답변하세요 (마크다운·백틱 없이 순수 JSON):
{{
  "hook":    "첫 시선을 잡는 핵심 후크 (1문장)",
  "appeal":  "소구포인트 유형 + 설명 (감성/이성/사회적증거/희소성/혜택 중 분류)",
  "target":  "타겟 고객 추정 (연령대, 관심사 등)",
  "message": "핵심 메시지 요약 (1문장)",
  "tags":    ["태그1", "태그2", "태그3"]
}}"""
...
with st.sidebar:
    ...
    # ── AI 설정 ──
    st.markdown('<div class="sidebar-label">🤖 AI 분석</div>', unsafe_allow_html=True)

    ai_enabled = st.toggle("소구포인트 분석 ON/OFF", value=st.session_state.ai_enabled)
    st.session_state.ai_enabled = ai_enabled

    if ai_enabled:
        if API_KEY:
            st.success("API Key 연결됨 ✓")
            st.caption("Claude Haiku · 소재 1개 ≈ ₩1 미만")
        else:
            st.error("API Key 없음 — Secrets 미설정")
            st.caption("Streamlit Cloud → Settings → Secrets에\nANTHROPIC_API_KEY 등록 필요")
    else:
    ...
    country     = st.selectbox("국가", ["KR","US","JP","GB","SG","AU"],
    ...
    filter_type = st.selectbox("소재 타입", ["전체","image","video_poster"],
                               label_visibility="collapsed")
    all_kws     = ["전체"] + list(dict.fromkeys(
                    a["keyword"] for a in st.session_state.assets))
    filter_kw   = st.selectbox("키워드 필터", all_kws,
                               label_visibility="collapsed")
    ...
    shown_for_export = get_visible(filter_type,
                                   filter_kw if filter_kw != "전체" else None,
                                   filter_star, filter_ai)
    starred_assets = [a for a in st.session_state.assets if a.get("starred")]

    if shown_for_export:
        st.download_button(
            "📥 현재 보드 CSV",
            data=get_csv(shown_for_export),
            file_name=f"adintel_{time.strftime('%Y%m%d_%H%M')}.csv",
            mime="text/csv", use_container_width=True)
    if starred_assets:
        st.download_button(
            "⭐ 즐겨찾기 CSV",
            data=get_csv(starred_assets),
    ...
    if st.session_state.search_log:
        ...
        for log in reversed(st.session_state.search_log[-8:]):
            ...
            st.markdown(
                ...
                f'<span class="{cls}">[{sym}] {log["keyword"]} — {log["count"]}건</span>'
                f'</div>',
                unsafe_allow_html=True)
...
if ai_enabled and API_KEY:
    ...
    <div class="ai-banner">
        ...
        <span class="ai-banner-text">
            AI 분석 <b>ON</b> — 소재 카드의 [AI 분석] 버튼으로 소구포인트를 분석하세요
            &nbsp;·&nbsp; Claude Haiku 사용 중 (저비용)
        </span>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div class="ai-banner ai-banner-warn">
        ...
        <span class="ai-banner-text">
            AI ON이지만 API Key 미설정 —
            Streamlit Cloud → 앱 Settings → Secrets →
            <b>ANTHROPIC_API_KEY</b> 등록 필요
        </span>
    </div>
    """, unsafe_allow_html=True)
...
if st.session_state.search_history:
    ...
    for kw in st.session_state.search_history[-20:]:
        ...
        html += (
            f'<span class="kw-tag"><span class="dot"></span>'
            f'{kw} <span style="opacity:.5">({cnt})</span></span>'
        )
    html += "</div>"
...
if not shown_assets:
    ...
    <div class="empty-state">
        ...
        <div class="empty-desc">
            키워드를 입력하고 검색하면<br>
            Meta Ad Library에서 광고 소재를 실시간으로 수집합니다.
        </div>
    </div>
    """, unsafe_allow_html=True)
else:
    ...
    for idx, item in enumerate(shown_assets):
        with cols[idx % col_count]:
            ...
            try:
                ...
                st.markdown(
                    '<div style="height:120px;background:#f0f2f5;'
                    'display:flex;align-items:center;justify-content:center;'
                    'color:#8a909e;font-size:11px;font-family:DM Mono,monospace">'
            ...
            cap_html   = (
                f'<div class="asset-caption">"{cap[:55]}{"..." if len(cap)>55 else ""}"</div>'
                if cap else ""
            )
            ...
            if analysis:
                ...
                <div class="ai-wrap">
                  <div class="ai-box">
                    <div class="ai-box-header">⚡ 소구포인트 분석</div>
            ...
            if bd is not None:
                with bd:
                    if st.button("AI 분석", key=f"ai_{item['id']}", use_container_width=True):
                        ...
                        if result:
                            ...
                            st.error("분석 실패 — API Key 또는 네트워크 확인")
...
ai_status = "AI ON · Haiku" if (ai_enabled and API_KEY) else "AI OFF"
st.markdown(f"""
<div style="border-top:2px solid var(--border);padding-top:14px;
  ...
  <span style="font-family:'DM Mono',monospace;font-size:10px;color:var(--muted)">
    ADINTEL v3.1 · META AD LIBRARY INTELLIGENCE · {ai_status}
  </span>
  <span style="font-family:'DM Mono',monospace;font-size:10px;color:var(--muted)">
    {time.strftime("%Y-%m-%d")}
  </span>
